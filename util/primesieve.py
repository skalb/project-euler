...
This module uses the Sieve of Eratosthenes to generate primes in range 1 to N
Original algorithm based on definition at: http://en.wikipedia.org/wiki/Sieve_of_Eratosthenes
Optimizations included from solution provided at: http://projecteuler.net/index.php?section=problems&id=3
'''
...
def getPrimes(n):
    ...
    for i in xrange(1, cross, 1):
        ...
        if sieve[i] == False:
            # We can now set each multiple of i to true since it can longer be prime
            # Sieving goes from candidate p^2 to sievebound with step p
            # If the nth element has value 2i + 1, then to get the index of p^2:
            # p to p^2 = (p^2 - 1)/2 
            #          = (4i^2 + 4i)/2 
            #          = 2i(i + 1)
            start = 2*i*(i+1)
            ...
            for j in xrange(start, sievebound, step):
                # Set the index j to true to mark this number as having
    ...
    # All candidates with no factors are the prime numbers    
